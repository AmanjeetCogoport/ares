...
//    @Query(
//        """
//        select organization_name,document_no as invoice_number,acc_type as invoice_type, null as shipment_id, null as shipment_type,null as doc_type,
//        amount_loc as invoice_amount, 'INR' as currency, amount_loc - pay_loc as balance_amount,transaction_date as invoice_date,due_date as invoice_due_date,case when due_date < now() then now()::date - due_date else 0 end as overdue_days,
//        replace(organization_id::text, '-','_') as organization_id
//        from account_utilizations
//        where acc_type = 'sinv'
//        """
//    )
//    suspend fun fetchInvoice(): List<CustomerInvoice>
    ...
    suspend fun generateOverallStats(zone: String?): OverallOutstanding

    @Query(
        """
        select 'Total' as month,sum(case when acc_type = 'sinv' then sign_flag*(amount_loc - pay_loc) else 0 end) as total_receivable_amount,
        abs(sum(case when acc_type = 'rec' then sign_flag*(amount_loc - pay_loc) else 0 end)) as total_collected_amount
        from account_utilizations
        where (:quarter is null or extract(quarter from transaction_date) = :quarter) and (:zone is null or zone_code = :zone)
        union all
        (select trim(to_char(date_trunc('month',transaction_date),'Month')) as month,sum(case when acc_type = 'sinv' then sign_flag*(amount_loc - pay_loc) else 0 end) as total_receivable_amount,
        abs(sum(case when acc_type = 'rec' then sign_flag*(amount_loc - pay_loc) else 0 end)) as total_collected_amount from account_utilizations
        where (:quarter is null or extract(quarter from transaction_date) = :quarter) and (:zone is null or zone_code = :zone)
        group by date_trunc('month',transaction_date)
        order by date_trunc('month',transaction_date) desc)
        """
    )
    suspend fun generateCollectionTrend(zone: String?, quarter: Int?): List<CollectionTrendModel>
