...
interface AccountUtilizationRepository : CoroutineCrudRepository<AccountUtilization, Long> {
    ...
    @Query(
        ...
        (SELECT count(distinct tagged_organization_id) from account_utilizations where acc_type in ('SINV','SDN','SCN') AND amount_loc - pay_loc <> 0 AND document_value in (:documentValue) AND document_status = 'PROFORMA') as customers_count_proforma,
        ...
        (SELECT count(distinct tagged_organization_id) from account_utilizations where acc_type in ('SINV','SDN','SCN') AND amount_loc - pay_loc <> 0 AND document_value in (:documentValue) AND document_status in ('FINAL','PROFORMA') AND due_date > now()::date ) as customers_count_due,
        ...
        (SELECT count(distinct tagged_organization_id) from account_utilizations where acc_type in ('SINV','SDN','SCN') AND amount_curr - pay_curr <> 0 AND document_value in (:documentValue) AND document_status in ('FINAL','PROFORMA') AND due_date <= now()::date  ) as customers_count_overdue,
        ...
        (SELECT count(distinct tagged_organization_id) from account_utilizations where acc_type in ('SINV','SDN','SCN') AND amount_loc - pay_loc <> 0 AND document_value in (:documentValue) AND document_status in ('FINAL','PROFORMA')) as customers_count_receivables,
        ...
        FROM account_utilizations
        WHERE acc_mode = 'AR' AND document_value in (:documentValue) AND tagged_organization_id IS NOT NULL
        """
    )
