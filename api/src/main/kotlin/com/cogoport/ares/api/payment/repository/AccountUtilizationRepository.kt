...
interface AccountUtilizationRepository : CoroutineCrudRepository<AccountUtilization, Long> {
    ...
    @Query(
        ...
        Select
            ...
            COALESCE(sum(case when s.source_id = au.document_no and s.source_type in ('CTDS','VTDS', 'CTDSP') then s.amount end), 0) as tds,
            COALESCE(sum(case when s.source_type in ('CTDS','VTDS', 'CTDSP') then s.amount end), 0) as settled_tds,
