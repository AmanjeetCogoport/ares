...
interface AccountUtilizationRepository : CoroutineCrudRepository<AccountUtilization, Long> {
    ...
    @Query(
        ...
        y as (
            ...
            sum(case when acc_type in ('SINV','SDN','SCN','RSINV') then sign_flag*(amount_loc - pay_loc) else 0 end) + sum(case when acc_type in ('REC', 'OPDIV', 'MISC', 'BANK', 'CONTR', 'INTER', 'MTC', 'MTCCV') and document_status = 'FINAL' then sign_flag*(amount_loc - pay_loc) else 0 end) as amount
            from account_utilizations
    ...
    @Query(
        """
            ...
            y as (
                ...
                sum(case when acc_type in ('SINV','SDN','SCN','RSINV') then sign_flag*(amount_loc - pay_loc) else 0 end) + sum(case when acc_type in ('REC', 'OPDIV', 'MISC', 'BANK', 'CONTR', 'INTER', 'MTC', 'MTCCV') and document_status = 'FINAL' then sign_flag*(amount_loc - pay_loc) else 0 end) as total_outstanding_amount 
                from account_utilizations
    ...
    @Query(
        ...
        with X as (
            ...
            sum(case when acc_type in ('SINV','SDN','SCN','RSINV') then sign_flag*(amount_loc - pay_loc) else 0 end) as open_invoice_amount,
            ...
            sum(case when acc_type in ('SINV','SDN','SCN','RSINV') then sign_flag*(amount_loc - pay_loc) else 0 end) + sum(case when acc_type in ('REC', 'OPDIV', 'MISC', 'BANK', 'CONTR', 'INTER', 'MTC', 'MTCCV') and document_status = 'FINAL' then sign_flag*(amount_loc - pay_loc) else 0 end) as outstandings,
            sum(case when acc_type in ('SINV','SDN','SCN','RSINV') and transaction_date >= date_trunc('month',(:date)::date) then sign_flag*amount_loc end) as total_sales,
    ...
    @Query(
        ...
        with X as (
            select 
            extract(month from date_trunc('month',(:date)::date)) as month,
            sum(case when acc_type in ('PINV','PDN','PCN','RPINV') then sign_flag*(amount_loc - pay_loc) else 0 end) as open_invoice_amount,
            ...
            sum(case when acc_type in ('PINV','PDN','PCN','RPINV') then sign_flag*(amount_loc - pay_loc) else 0 end) + sum(case when acc_type in ('PAY', 'OPDIV', 'MISC', 'BANK', 'CONTR', 'INTER', 'MTC', 'MTCCV') then sign_flag*(amount_loc - pay_loc) else 0 end) as outstandings,
            sum(case when acc_type in ('PINV','PDN','PCN','RPINV') and transaction_date >= date_trunc('month',transaction_date) then sign_flag*amount_loc end) as total_sales,
            case when date_trunc('month', :date::date) < date_trunc('month', now()) then date_part('days',date_trunc('month',(:date::date + '1 month'::interval)) - '1 day'::interval) 
            else date_part('days', now()::date) end as days
            from account_utilizations
    ...
    @Query(
        ...
        from account_utilizations
        where acc_type in ('SINV','SCN','SDN','REC', 'OPDIV', 'MISC', 'BANK', 'CONTR', 'INTER', 'MTC', 'MTCCV', 'RSINV') and acc_mode = 'AR' and document_status in ('FINAL', 'PROFORMA') 
