    ...
    @Query(
        """
            SELECT
                ...
                sum(
                    CASE WHEN acc_type IN (:accType) THEN
                        sign_flag * (amount_loc - pay_loc)
                    ELSE
                        0
                ...
                sum(
                    CASE WHEN due_date >= now()::date AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS not_due_count,
                sum(
                    CASE WHEN (now()::date - due_date) BETWEEN 0 AND 30 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS thirty_count,
                sum(
                    CASE WHEN (now()::date - due_date) BETWEEN 31 AND 45 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS forty_five_count,
                sum(
                    CASE WHEN (now()::date - due_date) BETWEEN 46 AND 60 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS sixty_count,
                sum(
                    CASE WHEN (now()::date - due_date) BETWEEN 61 AND 90 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS ninety_count,
                sum(
                    CASE WHEN (now()::date - due_date) BETWEEN 91 AND 180 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
                    END) AS one_eighty_count,
                sum(
                    CASE WHEN (now()::date - due_date) > 180 AND acc_type IN (:accType) THEN
                        1
                    ELSE
                        0
            ...
            WHERE
                ...
                AND acc_type IN (:accType)
                AND deleted_at IS NULL
            GROUP BY
                organization_id, entity_code, currency 
        """
    )
    suspend fun getOutstandingAgeingBucket(entityCode: Int, accType: List<AccountType>, orgId: String?): List<CustomerOutstandingAgeing>
        ...
        """
            SELECT
                ...
                sum(
                    ...
                    END) AS one_eighty_plus_led_amount,
                sum(
                    CASE WHEN acc_type in ('REC', 'CTDS') THEN
                        sign_flag * (amount_loc - pay_loc)
                    ELSE
                        0
                    END) AS total_led_outstanding,
                ...
                sum(
                    ...
                    END) AS total_curr_outstanding,
                sum(
                    CASE WHEN transaction_date >= now()::date AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS not_due_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) BETWEEN 0 AND 30 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS thirty_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) BETWEEN 31 AND 45 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS forty_five_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) BETWEEN 46 AND 60 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS sixty_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) BETWEEN 61 AND 90 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS ninety_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) BETWEEN 91 AND 180 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS one_eighty_count,
                sum(
                    CASE WHEN (now()::date - transaction_date) > 180 AND acc_type in ('REC', 'CTDS') THEN
                        1
                    ELSE
                        0
                    END) AS one_eighty_plus_count
